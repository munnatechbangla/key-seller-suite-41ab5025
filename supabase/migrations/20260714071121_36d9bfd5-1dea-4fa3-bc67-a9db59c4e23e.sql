...
-- Expose require_screenshot to public checkout
CREATE OR REPLACE FUNCTION public.list_public_payment_gateways()
RETURNS TABLE(id uuid, slug text, name text, type text, logo_url text, description text, is_enabled boolean, mode text, sort_order integer, config jsonb)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT g.id, g.slug, g.name, g.type::text, g.logo_url, g.description, g.is_enabled, g.mode::text, g.sort_order,
    CASE WHEN g.type::text = 'manual' THEN
...
-- Extend submit_manual_payment_proof to accept screenshot_url
DROP FUNCTION IF EXISTS public.submit_manual_payment_proof(text, text, jsonb, text, text);
...
BEGIN
  ...
  INSERT INTO public.manual_payment_submissions(
    order_id, gateway_id, gateway_slug, user_id, email, amount, currency, note, status, field_values, screenshot_url
  ) VALUES (
    ord.id, gw.id, gw.slug, ord.user_id, COALESCE(ord.email, _email),
    ord.total, ord.currency, NULLIF(_note, ''), 'pending',
    COALESCE(_field_values, '{}'::jsonb), NULLIF(_screenshot_url, '')
  ) RETURNING id INTO inserted_id;

  RETURN jsonb_build_object('ok', true, 'id', inserted_id);
END; $$;
REVOKE ALL ON FUNCTION public.submit_manual_payment_proof(text, text, jsonb, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_manual_payment_proof(text, text, jsonb, text, text, text) TO anon, authenticated, service_role;
