...
function visibilityClass(v) {
    ...
    if (v.desktop === false)
        cls.push("lg:hidden");
    if (v.tablet === false)
        cls.push("hidden md:max-lg:hidden");
    if (v.mobile === false)
        cls.push("max-md:hidden");
    return cls.join(" ");
}
function SectionWrap({ style, children }) {
    return (<section className={cn("py-6", visibilityClass(style?.visibility), style?.custom_class)} style={wrapperStyle(style)}>
      <div className={containerClass(style?.container_width)}>{children}</div>
    </section>);
}
export function ProductLayoutRenderer({ product, sections }) {
    return (<>
      {sections.map((s) => (<RenderProductSection key={s.id} section={s} product={product}/>))}
    </>);
}
function RenderProductSection({ section, product }) {
    ...
    switch (section.section_type) {
        case "breadcrumb":
            return (<SectionWrap style={style}>
        ...
        case "title":
          ...
          {c.show_rating && product.rating > 0 && (<div className="flex items-center gap-1 mt-2 text-sm">
        ...
        case "features": {
          ...
          <ul className="grid md:grid-cols-2 gap-2">
            {items.map((it, i) => (<li key={i} className="flex items-start gap-2"><Check className="h-4 w-4 mt-1 text-primary shrink-0"/><span>{it}</span></li>))}
          </ul>
        </SectionWrap>);
        }
        case "specifications": {
          ...
          <dl className="divide-y border rounded-md">
            {items.map((it, i) => (<div key={i} className="grid grid-cols-2 p-3">
        ...
        case "faq": {
            const items = c.items ?? product.faqs ?? [];
            return (<SectionWrap style={style}>
          {section.title && <h2 className="text-2xl font-bold mb-3">{section.title}</h2>}
          <div className="space-y-2">
            {items.map((f, i) => (<details key={i} className="border rounded-md p-3"><summary className="font-medium cursor-pointer">{f.q}</summary><p className="mt-2 text-muted-foreground">{f.a}</p></details>))}
          </div>
        </SectionWrap>);
        }
        case "reviews":
        ...
        case "trust_badges": {
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {items.map((it, i) => (<div key={i} className="flex items-center gap-2 text-sm text-muted-foreground"><Lock className="h-4 w-4"/>{it}</div>))}
          </div>
        </SectionWrap>);
        }
        case "cta":
...
function PurchaseCard({ product, config }) {
...
function RelatedProductsBlock({ slug, title, limit }) {
...
function RecentlyViewedBlock({ currentSlug, title, limit }) {
