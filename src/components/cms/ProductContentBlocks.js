...
function Icon({ name, className }) {
    if (!name)
        return null;
    const Cmp = LucideIcons[name] ?? Check;
    return <Cmp className={cn("h-4 w-4", className)}/>;
}
export function ProductContentBlocks({ blocks, product }) {
    return (<div className="space-y-6">
      {blocks.map((b) => <RenderBlock key={b.id} block={b} product={product}/>)}
    </div>);
}
function RenderBlock({ block, product }) {
    const c = block.json_content ?? {};
    switch (block.block_type) {
        case "rich_text":
            return (<div className="prose prose-sm max-w-none dark:prose-invert" dangerouslySetInnerHTML={{ __html: c.html ?? "" }}/>);
        case "heading": {
        ...
        case "quote":
            return (<blockquote className="border-l-4 pl-4 italic text-muted-foreground">
        ...
        case "code":
            return (<pre className="bg-muted rounded-md p-4 overflow-x-auto text-sm">
          <code className={`language-${c.language ?? "text"}`}>{c.code}</code>
        </pre>);
        case "image":
            return (<figure style={{ textAlign: c.align ?? "center" }}>
        ...
        case "gallery":
          ...
          {(c.images ?? []).map((img, i) => (<img key={i} src={img.url ?? img} alt={img.alt ?? ""} className="w-full h-40 object-cover rounded-md"/>))}
        </div>);
        case "video":
            return (<video src={c.url} controls autoPlay={!!c.autoplay} loop={!!c.loop} muted={c.muted !== false} className="w-full rounded-md"/>);
        case "youtube":
            return (<div className="aspect-video w-full">
          <iframe className="w-full h-full rounded-md" src={`https://www.youtube.com/embed/${c.video_id}${c.autoplay ? "?autoplay=1&mute=1" : ""}`} allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/>
        </div>);
        case "vimeo":
            return (<div className="aspect-video w-full">
          <iframe className="w-full h-full rounded-md" src={`https://player.vimeo.com/video/${c.video_id}${c.autoplay ? "?autoplay=1&muted=1" : ""}`} allow="autoplay; fullscreen; picture-in-picture" allowFullScreen/>
        </div>);
        case "accordion":
            return (<div className="space-y-2">
          {(c.items ?? []).map((it, i) => (<details key={i} className="border rounded-md p-3">
        ...
        case "faq":
            return (<div className="space-y-2">
          {(c.items ?? []).map((f, i) => (<details key={i} className="border rounded-md p-3">
              <summary className="font-medium cursor-pointer">{f.q}</summary>
              <p className="mt-2 text-muted-foreground">{f.a}</p>
            </details>))}
        </div>);
        case "icon_list":
            return (<ul className="grid md:grid-cols-2 gap-2">
          {(c.items ?? []).map((it, i) => (<li key={i} className="flex items-start gap-2">
        ...
        case "feature_list":
          ...
          {(c.items ?? []).map((it, i) => (<div key={i} className="border rounded-lg p-4">
        ...
        case "comparison_table":
            return (<div className="overflow-x-auto"><table className="w-full text-sm border rounded-md">
          <thead className="bg-muted/40"><tr>{(c.headers ?? []).map((h, i) => <th key={i} className="p-3 text-left">{h}</th>)}</tr></thead>
          <tbody>{(c.rows ?? []).map((r, i) => (<tr key={i} className="border-t">{r.map((cell, j) => <td key={j} className="p-3">{cell}</td>)}</tr>))}</tbody>
        </table></div>);
        case "pricing_table":
            return (<div className="grid md:grid-cols-3 gap-4">
          {(c.plans ?? []).map((p, i) => (<div key={i} className="border rounded-lg p-6 flex flex-col">
              <div className="font-semibold">{p.name}</div>
              <div className="text-3xl font-bold my-2">{p.price}</div>
              <ul className="text-sm space-y-1 flex-1">
                {(p.features ?? []).map((f, j) => (<li key={j} className="flex items-start gap-2"><Check className="h-4 w-4 mt-0.5 text-primary"/>{f}</li>))}
              </ul>
        ...
        case "statistics":
          ...
          {(c.items ?? []).map((it, i) => (<div key={i} className="border rounded-lg p-4 text-center">
        ...
        case "timeline":
            return (<ol className="border-l-2 pl-4 space-y-4">
          {(c.items ?? []).map((it, i) => (<li key={i}>
        ...
        case "steps":
          ...
          {(c.items ?? []).map((it, i) => (<li key={i} className="flex gap-3">
        ...
        case "alert": {
            ...
            return (<div className={cn("border-l-4 rounded-md p-4 flex gap-3", map[tone] ?? map.info)}>
        ...
        case "button":
            return (<div style={{ textAlign: c.align ?? "left" }}>
        ...
        case "download_card":
            return (<div className="border rounded-lg p-4 grid grid-cols-2 md:grid-cols-4 gap-4">
