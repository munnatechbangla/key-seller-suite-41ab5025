...
export const landingDeleteSectionFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context);
    const { error } = await context.supabase.from("landing_page_sections").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const landingDeleteTemplateFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context);
    const { error } = await context.supabase.from("landing_page_templates").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
