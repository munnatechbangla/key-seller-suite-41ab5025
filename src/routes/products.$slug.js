...
import { productQuery, relatedQuery, useProduct, useRelated, useProductsBySlugs } from "@/lib/catalog";
import { supabase } from "@/integrations/supabase/client";
import { useEffect, useState, useRef } from "react";
import { Star, ShoppingCart, Zap, Heart, Check, GitCompare, Truck, Lock, Package, Shield } from "lucide-react";
...
import { ProductCustomFields } from "@/components/site/ProductCustomFields";
...
import { ProductLayoutRenderer } from "@/components/cms/ProductLayoutRenderer";
import { useServerFn } from "@tanstack/react-start";
import { useQuery } from "@tanstack/react-query";
import { useSettings } from "@/lib/cms/settings";
import { productBlocksPublicFn } from "@/lib/product-blocks.functions";
import { ProductContentBlocks } from "@/components/cms/ProductContentBlocks";
...
export const Route = createFileRoute("/products/$slug")({
    ...
    head: ({ loaderData, params }) => {
        ...
        if (seo?.og_description) {
            const idx = meta.findIndex((m) => m.property === "og:description");
            if (idx >= 0)
                meta[idx] = { property: "og:description", content: seo.og_description };
        }
        if (seo?.twitter_title)
            meta.push({ name: "twitter:title", content: seo.twitter_title });
        if (seo?.twitter_description)
            meta.push({ name: "twitter:description", content: seo.twitter_description });
        if (seo?.twitter_image)
            meta.push({ name: "twitter:image", content: seo.twitter_image });
        meta.push({ property: "product:price:amount", content: p.price.toFixed(2) }, { property: "product:price:currency", content: p.currency_code || p.currency || currencyCode }, { property: "og:price:amount", content: p.price.toFixed(2) }, { property: "og:price:currency", content: p.currency_code || p.currency || currencyCode }, { property: "product:availability", content: (p.stock ?? 1) > 0 ? "in stock" : "out of stock" });
        const canonicalHref = seo?.canonical_url || undefined;
        return {
            meta,
            links: [canonicalHref ? { rel: "canonical", href: canonicalHref } : canonicalLink(path)],
            scripts,
        };
    },
    component: ProductPage,
    notFoundComponent: () => (<div className="min-h-screen grid place-items-center p-8 text-center">
      <div>
...
function ProductPage() {
    ...
    const dynamicLayout = layoutQuery.data;
...
function LegacyProductPage() {
    ...
    const richBlocks = (blocksQuery.data ?? []);
    ...
    const addToCart = () => { if (!guardCustomFields())
        return; cart.add(product, qty); toast.success(`${product.name} added to cart`); };
    const navigate = useNavigate();
    const buyNow = () => { if (!guardCustomFields())
        return; cart.add(product, qty); navigate({ to: "/checkout" }); };
    ...
    const featuredImage = galleryImages.find((g) => g.is_primary)?.url ??
        galleryImages[0]?.url ??
    ...
    return (<div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-10 grid grid-cols-[minmax(0,1fr)] lg:grid-cols-2 gap-10">
        <div className="min-w-0 space-y-4">
          <div className="relative aspect-square rounded-3xl bg-gradient-to-br from-primary/15 via-secondary/15 to-accent/15 grid place-items-center overflow-hidden shadow-elegant">
            <ProductThumb src={heroImage} emoji={product.emoji} alt={activeVariant?.name || product.name} size={800} className="h-full w-full object-cover animate-fade-in bg-transparent"/>
            {product.badge && (<span className="absolute top-4 left-4 px-3 py-1.5 rounded-full text-xs font-bold bg-gradient-primary text-primary-foreground shadow-elegant">
                {product.badge}
              </span>)}
            {off > 0 && (<span className="absolute top-4 right-4 px-3 py-1.5 rounded-full text-xs font-bold bg-accent text-accent-foreground">
                -{off}% OFF
              </span>)}
          </div>
          {galleryImages.length > 0 ? (<div className="grid grid-cols-4 gap-3">
              {galleryImages.slice(0, 8).map((img) => {
                const isActive = (activeImage ?? featuredImage) === img.url;
                return (<button key={img.id} type="button" onClick={() => setActiveImage(img.url)} className={cn("aspect-square rounded-xl overflow-hidden bg-card border transition-smooth", isActive ? "border-primary ring-2 ring-primary/40" : "border-border hover:border-primary")}>
                    <ProductThumb src={img.url} emoji={product.emoji} alt={img.alt ?? product.name} size={200} className="w-full h-full object-cover"/>
                  </button>);
            })}
            </div>) : (<div className="grid grid-cols-4 gap-3">
              {[product.emoji, "✨", "🔐", "⚡"].map((e, i) => (<div key={i} className="aspect-square rounded-xl bg-card border border-border grid place-items-center text-4xl">{e}</div>))}
            </div>)}
        </div>
        ...
        <div className="min-w-0 space-y-5">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <SaleBadges product={product} extra={{ stock: product.stock ?? null, salesCount: product.salesCount ?? null, createdAt: product.createdAt ?? null }} max={4}/>
            <LiveVisitorsCounter surface="product" seed={product.slug}/>
          </div>

          <p className="text-muted-foreground leading-relaxed">{product.short}</p>

          {hasAttrs ? (<VariantSelector product={product} onVariantChange={setActiveVariant} beforeAdd={validateCustomFields} beforeButtons={<ProductCustomFields ref={customFieldsRef} productSlug={product.slug}/>}/>) : (<>
              <div className="rounded-2xl bg-gradient-to-br from-primary/5 to-secondary/5 border border-primary/20 p-5 flex items-end gap-4 flex-wrap">
                <div>
                  ...
                  {product.oldPrice && (<div className="flex gap-2 items-center mt-1">
              ...
              <ul className="space-y-2">
                {product.features?.slice(0, 4).map((f) => (<li key={f} className="flex items-start gap-2 text-sm">
          ...
          <div className="flex min-w-0 gap-2 flex-wrap text-sm">
            ...
            <div className="min-w-0 sm:ml-auto">
              <ShareButtons path={`/products/${product.slug}`} title={product.name} description={product.short ?? undefined}/>
            </div>
          </div>

          <div className="grid grid-cols-3 gap-3 pt-4 border-t border-border">
            {[{ Icon: Truck, t: "Instant Delivery" }, { Icon: Shield, t: "Full Warranty" }, { Icon: Lock, t: "Secure Payment" }].map((b) => (<div key={b.t} className="text-center text-xs">
      ...
      <div className="container mx-auto px-4 pb-12">
        <div className="flex gap-1 border-b border-border overflow-x-auto">
          {["desc", "specs", "reviews", "faq"].map((t) => (<button key={t} onClick={() => setTab(t)} className={`px-5 py-3 text-sm font-semibold whitespace-nowrap border-b-2 transition-smooth ${tab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
              {{ desc: "Description", specs: "Specifications", reviews: "Reviews", faq: "FAQ" }[t]}
            </button>))}
        </div>

        <div className="py-8">
          {tab === "desc" && (richBlocks.length > 0 ? (<ProductContentBlocks blocks={richBlocks} product={product}/>) : (<div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <div className="text-muted-foreground leading-relaxed prose prose-sm max-w-none dark:prose-invert" dangerouslySetInnerHTML={{ __html: formatDescription(product.description || "") }}/>
                  <h3 className="font-semibold mt-6">Key features</h3>
                  <ul className="space-y-2">
                    {product.features?.map((f) => (<li key={f} className="flex items-start gap-2 text-sm"><Check className="h-4 w-4 text-emerald-600 mt-0.5"/> {f}</li>))}
                  </ul>
                </div>
                <div className="rounded-2xl bg-card border border-border p-6 space-y-3">
                  ...
                  <ul className="space-y-2">
                    {product.included?.map((f) => (<li key={f} className="flex items-start gap-2 text-sm"><Check className="h-4 w-4 text-primary mt-0.5"/> {f}</li>))}
                  </ul>
          ...
          {tab === "specs" && (<div className="rounded-2xl bg-card border border-border overflow-hidden max-w-2xl">
              {Object.entries(product.specs ?? {}).map(([k, v], i) => (<div key={k} className={`grid grid-cols-2 px-5 py-3 text-sm ${i % 2 ? "bg-muted/40" : ""}`}>
                  <span className="font-semibold">{k}</span>
                  <span className="text-muted-foreground">{v}</span>
                </div>))}
            </div>)}
          {tab === "reviews" && <ReviewsSection productId={product.id}/>}
          {tab === "faq" && (<div className="space-y-3 max-w-3xl">
              {product.faqs?.map((f) => (<details key={f.q} className="rounded-2xl bg-card border border-border p-5 group">
        ...
        <FrequentlyBoughtTogether current={product} candidates={related} currentVariant={activeVariant} currentHasAttributes={hasAttrs || !!product.hasAttributes}/>
        ...
        {recentProducts.length > 0 && (<section className="mt-12">
